...
push(value) {
  ...
  partitionList(x) {
    let listSmaller = new LinkedList(0);
    let listEqualOrBigger = new LinkedList(0);

    listSmaller.makeEmpty();
    listEqualOrBigger.makeEmpty();

    let current = this.head;

    while (current) {
      if (current.value < x) {
        listSmaller.push(current.value);
      } else {
        listEqualOrBigger.push(current.value);
      }
    ...
    if (listSmaller.head) {
      let listSmallerTail = listSmaller.head;
      while (listSmallerTail.next) {
        listSmallerTail = listSmallerTail.next;
      }
      listSmallerTail.next = listEqualOrBigger.head;
      this.head = listSmaller.head;
    } else if (listEqualOrBigger.head) {
      this.head = listEqualOrBigger.head;
    }
